...
// Base path — change this if your project folder is named differently
$base = "/ICS_APP_DEV1";
?>
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <span class="badge bg-warning text-dark p-2 mt-1">
        ...
        <div>
            <?php if ($current_user): ?>
                ...
                <a href="<?php echo $base; ?>/authentication/logout.php" class="login-btn-top">Logout</a>
            <?php else: ?>
    ...
    <div class="scroll-container mb-5">
        <?php if ($history_matches && $history_matches->num_rows > 0): ?>
            <?php while ($h = $history_matches->fetch_assoc()):
                $h_img = !empty($h['home_p']) ? $base . "/uploads/" . htmlspecialchars($h['home_p']) : "https://via.placeholder.com/50";
                $a_img = !empty($h['away_p']) ? $base . "/uploads/" . htmlspecialchars($h['away_p']) : "https://via.placeholder.com/50";
                if ($h['winner_id'] == 0) {
                    $winner_display = "DRAW";
                } else {
            ...
            <div class="history-card">
                ...
                <div class="text-center mt-3 small text-white-50">
                    ...
                    <?php echo date('M d, Y', strtotime($h['reservation_date'] ?? 'today')); ?>
                </div>
            </div>
            <?php endwhile; ?>
        <?php else: ?>
        ...
        <?php if ($recent_matches && $recent_matches->num_rows > 0): ?>
            <?php while ($row = $recent_matches->fetch_assoc()):
                ...
                $chal_q = "SELECT t.team_name, t.team_photo FROM match_requests mr 
                           ...
                           WHERE mr.reservation_id = ? AND mr.status = 'accepted' LIMIT 1";
                $chal_stmt = $conn->prepare($chal_q);
                $chal_stmt->bind_param("i", $res_id);
                $chal_stmt->execute();
                $challenger = $chal_stmt->get_result()->fetch_assoc();
                $opp_name   = $challenger['team_name'] ?? "TBD";
                $opp_photo  = !empty($challenger['team_photo']) ? $base . "/uploads/" . htmlspecialchars($challenger['team_photo']) : "https://via.placeholder.com/50?text=?";
                $home_photo = !empty($row['team_photo'])        ? $base . "/uploads/" . htmlspecialchars($row['team_photo'])        : "https://via.placeholder.com/50?text=Team";
            ?>
            <div class="recent-card">
                ...
                <div class="small fw-bold text-truncate">
                    <?php echo htmlspecialchars($row['team_name']); ?> vs <?php echo htmlspecialchars($opp_name); ?>
                </div>
    ...
    <!-- ALL TEAMS -->
    <h4 class="mb-3 text-warning">
        <i class="bi bi-people-fill"></i> All Teams
    </h4>
    <div class="scroll-container mb-5">
        <?php if ($all_teams && $all_teams->num_rows > 0): ?>
            <?php while ($t = $all_teams->fetch_assoc()):
                $t_photo = !empty($t['team_photo']) ? $base . "/uploads/" . htmlspecialchars($t['team_photo']) : "https://via.placeholder.com/80?text=Team";
            ?>
            <div class="team-card">
                <img src="<?php echo $t_photo; ?>" style="width:70px; height:70px; object-fit:cover; border-radius:10px; margin-bottom:10px;">
                <div class="fw-bold small text-truncate"><?php echo strtoupper(htmlspecialchars($t['team_name'])); ?></div>
                <small class="text-white-50" style="font-size:0.7rem;"><?php echo htmlspecialchars($t['game_type']); ?></small>
            </div>
            <?php endwhile; ?>
        <?php else: ?>
            <div class="empty-state">
    ...
    <div class="mt-5 pb-5">
        <a href="<?php echo $base; ?>/authentication/register.php?sid=<?php echo urlencode($current_user); ?>" class="btn btn-warning fw-bold px-4">
